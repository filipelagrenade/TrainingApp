 ...
 *
 * This is a faithful mirror of `backend/src/lib/calories.ts` so the live
 * preview shown while logging matches the server's stored estimate. Keep the
 * two in lock-step — the unit test (`tests/unit/calories.test.ts`) anchors the
 * same cases the backend does so drift is caught.
 *
 ...
 *
 * No side effects, deterministic. Never throws — always returns a non-negative
 * kcal value.
 */
